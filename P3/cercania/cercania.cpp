...
int main(int argc, char **argv){
    ...
    for(int i = 0; i < ciudades.size(); i++){
        ...
        int coste_total;
        ciudades_visitadas.push_back(ciudades[i]);

        for(int j=0;j<ciudades.size();j++){
            if(j!=i){
                ciudades_por_visitar.push_back(ciudades[j]);
            }
        }
        ...
        while(!ciudades_por_visitar.empty()){
            ...
            ciudades_por_visitar.erase(ciudades_por_visitar.begin()+posicion_minima);

        }

        coste_total = calcularCoste(ciudades_visitadas,matrizDistancias);
        if(coste_total<coste_total_minimo){
