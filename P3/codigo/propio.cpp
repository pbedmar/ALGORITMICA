...
//Función que sustituye la posicioón de dos ciudades entre sí, y las ciudades que hay 
// entre ellas la ordena de forma inversa.
...
int main(int argc, char **argv){
    ...
    while(contador < 100){                                                                  //La condicion de salida son 100 iteraciones sin que haya un cambio efectivo en el
        int coste_minimo = calcularCoste(ciudades_definitivas,matrizDistancias);            //vector de ciudades definitivas.
        for(int i=0; i<ciudades_definitivas.size(); i++){
            for(int j=i+1;j<ciudades_definitivas.size(); j++){
                swap_vector(ciudades_provisional, i, j);                                    // Ejecutamos la funcion para realizar los cambios entre las ciudades.
                int coste_total = calcularCoste(ciudades_provisional,matrizDistancias);

                if(coste_total < coste_minimo){                                             // Si el coste de al realizar el swap es menor, se realizan los cambios.
                    ciudades_definitivas = ciudades_provisional;
                    coste_minimo = coste_total;
                    contador = 0;                                                           // si se produce un cambio efectivo se reinicia el contador.
                }
            }
        }
        contador++;
