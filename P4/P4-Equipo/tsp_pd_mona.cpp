...
void generate(int k, vector<bool> & my_set,vector<vector<int> >& subconjuntos, int N)
{
	if(k == N)
	{
        vector<int> sub;
	...
	{
		my_set[k] = true;
		generate(k + 1, my_set, subconjuntos, N);
...
vector<vector<casillaDinamica>> obtenerMatrizDinamica(vector<vector<double>> matrizDistancias, vector< vector<int> > subconjuntos)
{

//Matriz a rellenar
    vector<vector<casillaDinamica>> matrizDinamica(4, vector<casillaDinamica>(subconjuntos.size()));
...
int main(int argc, char **argv){
    ...
    */
    ...
    vector<vector<double> > matrizDistancias(3);
    for(int i=0; i < matrizDistancias.size(); i++){
        for(int j=0; j < 3; j++){
            cout << "fila " << i << " col" << j << ": ";
            double dist;
            cin >> dist;
            matrizDistancias[i].push_back(dist);
        }
    }
    ...
    // vector de booleanos
    vector<bool> booleanos(4);
    
    // Llamar a la función que forma los subconjuntos
    generate(1, booleanos, subconjuntos, 3);

    sort(subconjuntos.begin(), subconjuntos.end(), ComparaTamanios);

    vector<vector<casillaDinamica> > matriz = obtenerMatrizDinamica(matrizDistancias, subconjuntos);
