...
int main(int argc, char **argv){
	...
	double max_norte=0, max_este=0, max_sur=ciudades[0].coord_y;
	ciudad c_norte, c_este, c_sur;

    //Selecionamos las 3 ciudades iniciales
    ...
    //insertamos todas las ciudades que no sean las iniciales
    ...
    //insertamos las ciudades iniciales
    ...
    while(!ciudades_por_visitar.empty()){
    	...
    	for(int k=0;k<ciudades_por_visitar.size();k++){
    		...
    		for(int i=0;i<ciudades_visitadas.size();i++){
    			...
    			aux.insert(aux.begin()+i,nueva);                 //Comprobamos la ciudad más eficiente a insertar en nuestro vector de ciudades visitadas.
    			coste = calcularCoste(aux,matrizDistancias);


    			if(coste < minimo){                              //Comprobamos la posición donde el coste es menor.
    				minimo = coste;
    		...
    		aux = ciudades_visitadas;             
    		aux.push_back(ciudades_por_visitar[k]);               //La insertamos al final para comprobar su coste.
    		coste = calcularCoste(aux,matrizDistancias);


			if(coste < minimo){                                   //Comprobamos que la posición más eficiente sea la última.
				minimo = coste;
				minima_ciudad = ciudades_por_visitar[k];
    	...
    	ciudades_visitadas.insert(ciudades_visitadas.begin()+posicion_minima_visitadas,minima_ciudad); //insertamos la ciudad en la posición optima y que menor coste de.
    	ciudades_por_visitar.erase(ciudades_por_visitar.begin()+posicion_minima_por_visitar); //Borramos la ciudad seleccionada del vector por visitar
